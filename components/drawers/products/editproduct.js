...
import { getSingleProduct , updatestocksingleProduct, clearsingproAction, editSingleProduct } from "@/reducer/SingleProductSlice";
...
export default function EditProductDrawer({open ,  handleClose, id}){
...
useEffect(()=>{
    if(productsuccessData && singleproductData && open){
      ...
      tagid,
      unitid,
      hasVariation } = singleproductData[0]
      const fieldsToUpdate = {
        ...
        markedprice: markedprice? markedprice : 0,
        inStockQuantity: inStockQuantity? inStockQuantity : 0
    };

      if(singleproductData[0].hasVariation){
        setVariations(singleproductData[0].variations);
        console.log(singleproductData[0].variations , 'var');
      }
      console.log(singleproductData[0] , 'singleproduct');
      if(singleproductData[0].discountdetails){
        console.log(singleproductData[0].discountdetails , 'discount');
        setDateRange([singleproductData[0].discountdetails.startdate, singleproductData[0].discountdetails.enddate]);
        console.log(singleproductData[0].discountdetails.type, 'typeeee');
        setDiscountId(singleproductData[0].discountdetails._id);
        if (['percent', 'fixed'].includes(singleproductData[0].discountdetails.type)) {
          formik.setFieldValue('type', singleproductData[0].discountdetails.type);
      }
      console.log(singleproductData[0].discountdetails.amount);
        formik.setFieldValue('discount', singleproductData[0].discountdetails.amount);

      }
      formik.setValues(fieldsToUpdate); 
      console.log(singleproductData[0], 'prdoo');
    }
  },[productsuccessData]);
  ...
  formik.setFieldValue('enddate',endDate);
    ...
    const handleImageChange = (e) => {
      ...
      const validationSchema = Yup.object().shape({
        ...
        //     then: Yup.string().required('Discount is required when Date is filled'),
        //     otherwise: Yup.string().nullable()
        // }),
        // startdate: Yup.date()
        // .when('discount', {
        //     is: (discount) => discount && discount.length > 0,
        //     then: Yup.string().required('Date is required when Discount is filled'),
        //     otherwise: Yup.string().nullable()
        // })
});
    ...
    data.append('discountid', discountid);
      ...
      // console.log(variations , 'variation chellll');
      // data.append('variations', JSON.stringify(variations));
        dispatch(editSingleProduct({id, formData: data}
        ))
          resetForm(); 
          handleClose();
      }})
    ...
    if (varType.value) {
      ...
      setVarValues(varType.value); 
        ...
        disabled={formik.values.varvalueid === ''} 
        ...
        {variations.length>0 && variations.map((ele, index) => (
           <>
           {console.log(variations, 'cdscsdcdcdscsdcdsevbrevre')}
          <TableRow>
            ...
            <TableCell>
              ...
              pattern="^(?!0*$)\d+(\.\d+)?$" 
              name="markedprice" 
              value={ele.markedprice} 
              required 
              onChange={(event) => handleVariationUpdate(index, 'markedprice', event.target.value)}
