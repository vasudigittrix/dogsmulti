...
export default function EditTaxDrawer({open , id,  handleClose}){
    const dispatch = useDispatch();
    const taxeditsuccessData = useSelector((state)=> state.singletax.editsuccess);
    const taxsuccessData = useSelector((state)=> state.singletax.success);
    const taxcontentData = useSelector((state)=> state.singletax.content);
    ...
    useEffect(()=>{
        if(open){
          dispatch(getSingletax(
            {id}
          ))
        }
      },[open, id]);
    useEffect(()=>{
        if(taxsuccessData && taxcontentData){
            formik.setValues(taxcontentData);
            dispatch(clearsintaxAction());
        }
    },[taxsuccessData]);
    const initialvalues = {
        title: "",
