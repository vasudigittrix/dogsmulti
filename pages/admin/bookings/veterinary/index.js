...
import NewVeterinaryDrawer from "@/components/drawers/newbooking/veterinary";
...
export default function VetBookingPage(){
    ...
    //     }
    // ))
    // },[ boardingdelsucData, boardingupdatesucData, boardingupdatepaysucData, boardingcreatesucData, currentPage]);
    useEffect(()=>{
        if (searchQuery) {
   ...
   const BookingtypestatusHandler = (status)=>{
    ...
    return(
        ...
        {!loading &&  (
             <VetBookingTable data = {borbookings} selectedItems={selectedItems} setSelectedItems = {setSelectedItems}/>
        )
    }
       <NewVeterinaryDrawer
        open = {isModalOpen}
