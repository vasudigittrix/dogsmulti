...
import { getServicesByRole, clearSrvroleAction } from "@/reducer/Employees/ServiceByRole";
import { useSession, signIn, signOut } from "next-auth/react";
import ServiceStatusFilter from "@/components/shared/employee/ServiceStFilter";
export default function VetServicesList() {
  ...
  const servicessuccessData = useSelector((state)=> state.servicesbyrole.success);
  const servicescontentData = useSelector((state)=> state.servicesbyrole.content);
 ...
 useEffect(()=>{
  ...
  const handleApply = (selectedValue, selectedstatValue) => {
    ...
    return(
        <>
        <ToastContainer />
        <ServiceStatusFilter addcategoryHandler = {addcategoryHandler} selectedItems={selectedItems} handleApply = {handleApply} filterhandler = {servicestatusHandler}/>
        {!loading && 
